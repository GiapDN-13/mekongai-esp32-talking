...
function getStore() {
  if (!_store) {
    _store = require('../store/index').default;
  }
  return _store;
}
...
function sendRequest() {
    return {
        ...
        'send'() {
            ...
            // Convert locale code format: zh_CN -> zh-CN
            ...
            if (isNotNull(getStore().getters.getToken)) {
                this._header.Authorization = 'Bearer ' + (JSON.parse(getStore().getters.getToken)).token
            }
 ...
 * failCallback - business error callback
 * networkFailCallback - network error callback
 */
