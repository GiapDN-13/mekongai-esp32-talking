  ...
  <el-header class="header">
    <div class="header-container">
      ...
      <div class="header-left" @click="goHome">
        <img loading="lazy" alt="" src="@/assets/mekongai-logo.png" class="logo-img" />
        <img loading="lazy" alt="" src="@/assets/mekongai-text.png" class="brand-img" />
      </div>
...
export default {
  ...
  computed: {
    ...
    isSuperAdmin() {
      ...
      let s = u.superAdmin;
      if ((s === undefined || s === null || s === "") && u.super_admin !== undefined) {
        s = u.super_admin;
      }
      const n = Number(s);
      return n === 1;
    ...
    featureStatus() {
      ...
      const on = (key) => {
        if (!f || f[key] === undefined || f[key].enabled === undefined) {
          return true;
        }
        return !!f[key].enabled;
      };
      return {
        voiceClone: on("voiceClone"),
        knowledgeBase: on("knowledgeBase"),
      };
    },
    // 获取当前语言
    currentLanguage() {
    ...
    // Brand text logo (universal, no per-locale switching needed)
    brandTextIcon() {
      return require("@/assets/mekongai-text.png");
    },
    // 用户菜单选项
    userMenuOptions() {
      return [
  ...
  async mounted() {
    ...
    // 已登录时从服务端刷新用户信息，避免 localStorage 缺 superAdmin 或 super_admin=0 陈旧导致超级管理员菜单不显示
