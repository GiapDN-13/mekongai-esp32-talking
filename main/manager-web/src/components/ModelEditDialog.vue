<template>
  <el-dialog :visible.sync="dialogVisible" :close-on-click-modal="false" width="57%" center custom-class="custom-dialog"
    ...
    <div style="display: flex; justify-content: center; gap: 12px">
      <el-button @click="handleTest" :loading="testing" :disabled="testing">
...
export default {
  ...
  methods: {
    ...
    handleTest() {
      ...
      Api.model.testModelConfig(
        configJson,
        (res) => {
          this.testing = false;
          const data = res?.data?.data || {};
          const parts = [];
          if (data.qdrant) parts.push(`Qdrant: ${data.qdrant}`);
          if (data.embedding) parts.push(`Embedding: ${data.embedding}`);
          const msg = parts.join('\n') || 'No test results';
          const allOk = Object.values(data).every(v => v === 'OK' || v.startsWith('SKIP'));
          if (allOk) {
        ...
        () => {
          this.testing = false;
          this.$message.error('Kiểm tra thất bại — không kết nối được server');
        }
      );
    },
    handleSave() {
