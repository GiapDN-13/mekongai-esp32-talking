          ...
          <el-form :model="currentFunction" class="param-form">
            ...
            <el-form-item v-for="field in currentFunction.fieldsMeta" :key="field.key" :label="field.label"
              ...
              <el-input-number v-else-if="field.type === 'number'"
                :value="numberParamValue(currentFunction, field)"
                @change="val => handleParamChange(currentFunction, field.key, val)" />
...
export default {
  ...
  watch: {
    ...
    value(v) {
      ...
      if (v) {
        ...
        // 右侧默认指向第一个（merge default + Qdrant embedding để không trống vector/model）
        const first = this.selectedList[0] || null;
        if (first) {
          this.ensureParamsDefaults(first);
          this.fillMissingQdrantEmbeddingDefaults(first);
        }
        this.currentFunction = first;
  ...
  methods: {
    ...
    /** el-input-number cần số; fallback default từ fieldsMeta để không trống khi params thiếu */
    numberParamValue(fn, field) {
      if (!fn || !fn.params) return field.default !== undefined ? field.default : 0;
      const v = fn.params[field.key];
      if (v === undefined || v === null || v === '') {
        return field.default !== undefined ? field.default : 0;
      }
      return v;
    },
    ensureParamsDefaults(fn) {
      if (!fn || !fn.fieldsMeta || !fn.params) return;
      fn.fieldsMeta.forEach((field) => {
        const v = fn.params[field.key];
        const empty = v === undefined || v === null || v === '';
        if (empty && field.default !== undefined && field.default !== null) {
          this.$set(fn.params, field.key, field.default);
        }
      });
    },
    /** search_from_qdrant: chỉ Gemini embedding */
    fillMissingQdrantEmbeddingDefaults(fn) {
      if (!fn || fn.name !== 'search_from_qdrant' || !fn.params) return;
      const p = fn.params;
      const prov = String(p.embedding_provider || '').toLowerCase();
      if (!prov || prov === 'gemini') {
        const missing = (k) => p[k] === undefined || p[k] === null || p[k] === '';
        if (missing('embedding_model')) p.embedding_model = 'gemini-embedding-001';
        if (missing('vector_size')) p.vector_size = 768;
        if (missing('top_k')) p.top_k = 3;
        if (missing('score_threshold')) p.score_threshold = '0.55';
      }
    },
    handleFunctionClick(func) {
      if (this.selectedNames.includes(func.name)) {
        const tempFunc = this.tempFunctions[func.name];
        const next = tempFunc ? tempFunc : func;
        this.ensureParamsDefaults(next);
        this.fillMissingQdrantEmbeddingDefaults(next);
    ...
    handleParamChange(func, key, value) {
      ...
      const working = this.tempFunctions[func.name];
      working.params[key] = value;

      if (key === 'embedding_provider') {
        const params = working.params;
        const v = String(value || '').toLowerCase();
        if (v === 'gemini') {
          params.embedding_model = 'gemini-embedding-001';
          params.vector_size = 768;
          if (params.top_k === undefined || params.top_k === null || params.top_k === '') {
            params.top_k = 3;
          }
          if (params.score_threshold === undefined || params.score_threshold === null || params.score_threshold === '') {
            params.score_threshold = '0.55';
          }
        }
      }

      // Luôn trỏ currentFunction vào bản đã sửa — trước đây chỉ cập nhật tempFunctions nên UI không đổi
      if (this.currentFunction && this.currentFunction.name === func.name) {
        this.currentFunction = working;
      }
    },
    handleCheckboxChange(func, checked) {
      ...
      if (this.selectedList.length > 0) {
        ...
        const temp = this.tempFunctions[first.name];
        const next = temp ? temp : first;
        this.ensureParamsDefaults(next);
        this.fillMissingQdrantEmbeddingDefaults(next);
        this.currentFunction = next;
      } else {
    ...
    selectAll() {
      ...
      if (this.selectedList.length > 0) {
        const raw = JSON.parse(JSON.stringify(this.selectedList[0]));
        this.ensureParamsDefaults(raw);
        this.fillMissingQdrantEmbeddingDefaults(raw);
    ...
    saveSelection() {
      ...
      const selected = this.selectedList.map(f => {
        ...
        const src = modified || f;
        const clone = JSON.parse(JSON.stringify(src));
        this.ensureParamsDefaults(clone);
        this.fillMissingQdrantEmbeddingDefaults(clone);
        return {
          id: f.id,
          name: f.name,
          params: { ...clone.params }
