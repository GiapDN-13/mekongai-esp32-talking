...
export default {
    ...
    methods: {
        ...
        handleSubmit({ form, done }) {
            if (form.id) {
                Api.agent.updateAgentVoicePrint({ ...form, audioId: form.audioId }, ({ data }) => {
                    if (data.code === 0) {
                        this.$message.success({ message: this.$t('voicePrint.updateSuccess'), showClose: true });
                        this.dialogVisible = false;
                        this.fetchVoicePrints();
                    }
                    done && done();
                });
            } else if (form._directRegistered) {
                Api.agent.addAgentVoicePrint({
                    agentId: this.agentId,
                    audioId: form._speakerId,
                    sourceName: form.sourceName,
                    introduce: form.introduce,
                    directRegistered: true,
                    speakerId: form._speakerId
                }, ({ data }) => {
                    if (data.code === 0) {
                        this.$message.success({ message: this.$t('voicePrint.addSuccess'), showClose: true });
                        this.dialogVisible = false;
                        this.fetchVoicePrints();
                    }
                    done && done();
