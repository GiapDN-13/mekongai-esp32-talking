                ...
                <el-table-column :label="$t('knowledgeFileUpload.operation')" align="center" width="200">
                  <template slot-scope="scope">
                    <el-button size="mini" type="text" @click="handleParse(scope.row)"
                      :disabled="scope.row.parseStatusCode === 1" v-if="!scope.row.sliceCount || scope.row.sliceCount <= 0">
...
export default {
  ...
  methods: {
    ...
    updateDocumentStatuses: function (updatedFileList) {
      ...
      updatedFileList.forEach(updatedDoc => {
        ...
        if (existingDoc && existingDoc.parseStatusCode !== updatedDoc.parseStatusCode) {
          ...
          // 如果状态变为完成，启动切片数量检测
          if (updatedDoc.parseStatusCode === 2) {
    ...
    getParseStatusType: function (parseStatusCode) {
      // Backend: 0=pending, 1=parsing, 2=done, 3=failed, 4=partial(done_with_error)
    ...
    getParseStatusText: function (parseStatusCode) {
      switch (parseStatusCode) {
        ...
        case 4:
          return this.$t('knowledgeFileUpload.statusPartial') || this.$t('knowledgeFileUpload.statusFailed');
        default:
          return this.$t('knowledgeFileUpload.statusNotStarted');
      }
    },
    ...
    runRetrievalTest: function () {
      ...
      // Chuẩn backend /retrieval-test: query, top_k, score_threshold
      const requestData = {
        query: this.retrievalTestForm.question.trim(),
        top_k: 5,
        score_threshold: 0.2
      };
      ...
      KnowledgeBaseAPI.retrievalTest(this.datasetId, requestData,
        ({ data }) => {
          ...
          if (data && data.code === 0) {
            ...
            const results = payload.results || payload.chunks || [];

            // Normalize backend shape for UI renderer (UI expects `chunks` + `similarity`)
            this.retrievalTestResult = {
              ...payload,
