            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(result, dict):
                        ...
                        self.logger.bind(tag=TAG).warning(f"API {url} response is not a JSON object")
                else:
                    self.logger.bind(tag=TAG).warning(f"API {url} request failed: HTTP {response.status_code}")
            except Exception as e:
                self.logger.bind(tag=TAG).error(f"Failed to fetch context from {url}: {e}")
        
        # 将所有格式化后的行拼接成一个字符串
        self.context_data = "\n".join(formatted_lines)
        if self.context_data:
            self.logger.bind(tag=TAG).debug(f"Injected dynamic context:\n{self.context_data}")
