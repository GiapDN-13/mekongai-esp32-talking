...
def write_json_file(file_path, data):  # TODO: dead code — remove (0 callers)
    """将数据写入 JSON 文件"""
...
def check_ffmpeg_installed() -> bool:
    ...
    try:
        ...
        # 构建基础错误提示
        error_msg = [
            "ERROR: ffmpeg is not working properly.\n",
            "建议您：",
            "1. 确认已正确激活 conda 环境；",
        ...
        if "libiconv.so.2" in stderr_output:
            error_msg.append("WARNING: Missing library: libiconv.so.2")
            ...
            error_msg.append("WARNING: ffmpeg executable not found.")
            error_msg.append("解决方法：在当前 conda 环境中执行：")
