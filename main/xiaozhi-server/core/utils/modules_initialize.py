...
def initialize_modules(
    ...
    if init_tts:
        ...
        logger.bind(tag=TAG).info(f"Module init OK: TTS={select_tts_module}")
    ...
    if init_llm:
        ...
        logger.bind(tag=TAG).info(f"Module init OK: LLM={select_llm_module}")
    ...
    if init_intent:
        ...
        logger.bind(tag=TAG).info(f"Module init OK: Intent={select_intent_module}")
    ...
    if init_memory:
        ...
        logger.bind(tag=TAG).info(f"Module init OK: Memory={select_memory_module}")
    ...
    if init_vad:
        ...
        logger.bind(tag=TAG).info(f"Module init OK: VAD={select_vad_module}")

    # 初始化ASR模块
    if init_asr:
        select_asr_module = config["selected_module"]["ASR"]
        modules["asr"] = initialize_asr(config)
        logger.bind(tag=TAG).info(f"Module init OK: ASR={select_asr_module}")
    return modules
...
def initialize_voiceprint(asr_instance, config):  # TODO: dead code — remove (connection.py uses VoiceprintProvider directly)
    """初始化声纹识别功能"""
    ...
    try:
        ...
        logger.bind(tag=TAG).info("Voiceprint dynamically enabled for ASR module")
        logger.bind(tag=TAG).info(f"Configured speakers: {len(voiceprint_config['speakers'])}")
        return True
    except Exception as e:
        logger.bind(tag=TAG).error(f"Dynamic voiceprint init failed: {str(e)}")
        return False
