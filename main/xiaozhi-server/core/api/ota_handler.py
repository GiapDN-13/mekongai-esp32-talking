...
class OTAHandler(BaseHandler):
    ...
    def _refresh_bin_cache_if_needed(self):
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(f"Firmware cache refresh failed: {e}")
    ...
    def generate_password_signature(self, content: str, secret_key: str) -> str:
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(f"MQTT password signature failed: {e}")
            return ""

    def _get_websocket_url(self, local_ip: str, port: int) -> str:
        """Get WebSocket URL for OTA response.
    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            self.logger.bind(tag=TAG).debug(f"OTA request body: {data}")

            device_id = request.headers.get("device-id", "")
            if device_id:
                self.logger.bind(tag=TAG).info(f"OTA device-id: {device_id}")
            else:
                raise Exception("OTA请求设备ID为空")

            client_id = request.headers.get("client-id", "")
            if client_id:
                self.logger.bind(tag=TAG).info(f"OTA client-id: {client_id}")
            else:
            ...
            if mqtt_gateway_endpoint:  # 如果配置了非空字符串
                ...
                try:
                    ...
                    self.logger.bind(tag=TAG).error(f"Failed to build MQTT username: {e}")
                    username = ""
                ...
                if signature_key:
                    ...
                    self.logger.bind(tag=TAG).warning("No MQTT signature key; password empty")
                ...
                self.logger.bind(tag=TAG).info(
                    f"No MQTT gateway; sending WebSocket config for device {device_id}"
                )
            ...
            try:
                ...
                self.logger.bind(tag=TAG).info(
                    f"Firmware lookup for model {device_model}: {len(candidates)} candidate(s)"
                )
                ...
                if chosen_url:
                    ...
                    self.logger.bind(tag=TAG).info(
                        f"Pushing firmware {chosen_version} to {device_id} (check server.vision_explain if URL wrong) -> {chosen_url} "
                    )
                else:
                    self.logger.bind(tag=TAG).info(
                ...
                self.logger.bind(tag=TAG).error(f"Firmware version check error: {e}")
            ...
            self.logger.bind(tag=TAG).error(f"OTA POST error: {e}")
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            self.logger.bind(tag=TAG).error(f"OTA GET error: {e}")
