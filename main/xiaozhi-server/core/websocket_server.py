...
class WebSocketServer:
    ...
    async def _handle_connection(self, websocket: websockets.ServerConnection):
        ...
        if headers.get("device-id", None) is None:
            ...
            if not request_path:
                self.logger.bind(tag=TAG).error("Could not get request path")
        ...
        try:
            ...
            try:
                ...
                self.logger.bind(tag=TAG).error(
                    f"Error forcing connection close on server: {close_error}"
                )
        ...
        try:
            async with self.config_lock:
                ...
                if new_config is None:
                    ...
                    return False
                self.logger.bind(tag=TAG).info("Fetched new config successfully")
                # 检查 VAD 和 ASR 类型是否需要更新
                update_vad = check_vad_update(self.config, new_config)
                update_asr = check_asr_update(self.config, new_config)
                self.logger.bind(tag=TAG).info(
                    f"VAD/ASR update check: vad={update_vad}, asr={update_asr}"
                )
                # 更新配置
                ...
                self.logger.bind(tag=TAG).info("Config reload task finished")
                return True
        except Exception as e:
