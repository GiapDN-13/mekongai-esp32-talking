...
async def check_direct_exit(conn: "ConnectionHandler", text):
    ...
    for cmd in cmd_exit:
        if text == cmd:
            conn.logger.bind(tag=TAG).info(f"Explicit exit command matched: {text}")
...
async def analyze_intent_with_llm(conn: "ConnectionHandler", text):
    ...
    if not hasattr(conn, "intent") or not conn.intent:
        conn.logger.bind(tag=TAG).warning("Intent recognition not initialized")
        return None
    ...
    try:
        ...
        if "function_call" in intent_data:
            ...
            conn.logger.bind(tag=TAG).debug(
                f"Intent result has function_call: {intent_data['function_call']['name']}"
            )
            function_name = intent_data["function_call"]["name"]
            if function_name == "continue_chat":
                return False

            # Let chat() + RAG Inline handle story searches
            # so the storyteller prompt and sliding window are used properly.
            if function_name == "search_from_qdrant":
                conn.logger.bind(tag=TAG).info(
                    "Bypassing tool-call path for search_from_qdrant → chat()"
                )
                return False
            ...
            def process_function_call():
                ...
                if result:
                    if result.action == Action.RESPONSE:  # 直接回复前端
                        ...
                        # Track narrated tail so RAG Inline "continue" knows where we left off
                        _st = getattr(conn, "_rag_story_state", None)
                        if _st and _st.get("story_id") and llm_result:
                            _st["last_narrated_tail"] = llm_result[-200:]
                    elif (
