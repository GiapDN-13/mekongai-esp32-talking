...
async def connect_mcp_endpoint(mcp_endpoint_url: str, conn=None) -> MCPEndpointClient:
    ...
    try:
        ...
        logger.bind(tag=TAG).error(f"MCP endpoint connection failed: {e}")
        return None
...
async def handle_mcp_endpoint_message(mcp_client: MCPEndpointClient, message: str):
    ...
    try:
        ...
        logger.bind(tag=TAG).debug(f"MCP endpoint message: {payload}")
        ...
        if "result" in payload:
            ...
            if msg_id == 1:  # mcpInitializeID
                logger.bind(tag=TAG).debug("MCP endpoint initialize response")
                if result is not None and isinstance(result, dict):
                    ...
                    if isinstance(server_info, dict):
                        ...
                        logger.bind(tag=TAG).info(
                            f"MCP endpoint server: name={name}, version={version}"
                        )
                else:
                    logger.bind(tag=TAG).warning(
                        "MCP endpoint initialize response empty or invalid"
                    )
                return

            elif msg_id == 2:  # mcpToolsListID
                logger.bind(tag=TAG).debug("MCP endpoint tools/list response")
                if (
                    result is not None
                    ...
                    if not isinstance(tools_data, list):
                        logger.bind(tag=TAG).error("Invalid tools list format")
                        return

                    logger.bind(tag=TAG).info(
                        f"MCP endpoint tools count: {len(tools_data)}"
                    )
                    ...
                    if next_cursor:
                        logger.bind(tag=TAG).info(
                            f"More tools, nextCursor: {next_cursor}"
                        )
                        ...
                        logger.bind(tag=TAG).info(
                            "All MCP endpoint tools loaded; client ready"
                        )
                        ...
                        logger.bind(tag=TAG).info(
                            f"MCP endpoint tools loaded: {len(mcp_client.tools)}"
                        )
                else:
                    logger.bind(tag=TAG).warning(
                        "MCP endpoint tools/list response empty or invalid"
                    )
                return
        ...
        # Handle method calls (requests from the endpoint)
            ...
            logger.bind(tag=TAG).info(f"MCP endpoint request: {method}")

        elif "error" in payload:
            error_data = payload["error"]
            error_msg = error_data.get("message", "unknown error")
            logger.bind(tag=TAG).error(f"MCP endpoint error response: {error_msg}")
        ...
        logger.bind(tag=TAG).error(f"MCP endpoint JSON parse failed: {e}")
    except Exception as e:
        logger.bind(tag=TAG).error(f"MCP endpoint message handling error: {e}")
        import traceback
...
async def send_mcp_endpoint_initialize(mcp_client: MCPEndpointClient):
    ...
    logger.bind(tag=TAG).info("Sending MCP endpoint initialize")
...
async def send_mcp_endpoint_tools_list(mcp_client: MCPEndpointClient):
    ...
    message = json.dumps(payload)
    logger.bind(tag=TAG).debug("Sending MCP endpoint tools/list")
    await mcp_client.send_message(message)
...
async def send_mcp_endpoint_tools_list_continue(
    ...
    message = json.dumps(payload)
    logger.bind(tag=TAG).info(f"Sending MCP endpoint tools/list with cursor: {cursor}")
    await mcp_client.send_message(message)
