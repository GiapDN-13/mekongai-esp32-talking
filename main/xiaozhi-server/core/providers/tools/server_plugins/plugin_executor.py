...
class ServerPluginExecutor(ToolExecutor):
    ...
    async def execute(
        ...
        try:
            # 根据工具类型决定如何调用
            # 所有插件函数都传入conn参数，因为大多数需要访问config和event loop
            if hasattr(func_item, "type"):
                func_type = func_item.type
                if func_type.code in [2, 3, 4, 5]:  # WAIT, CHANGE_SYS_PROMPT, SYSTEM_CTL, IOT_CTL
                    # Một số WAIT tools có logic đồng bộ (future.result/IO chờ),
                    # chạy trực tiếp sẽ block event loop và gây timeout giả.
                    if func_type.code == 2:  # WAIT
