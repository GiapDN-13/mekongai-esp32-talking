...
async def send_mcp_message(conn: "ConnectionHandler", payload: dict):
    ...
    try:
        ...
        logger.bind(tag=TAG).debug(f"MCP message sent: {message}")
...
async def handle_mcp_message(
    ...
    if not isinstance(payload, dict):
        logger.bind(tag=TAG).error("MCP message missing payload or invalid format")
        return
    ...
    if "result" in payload:
        ...
        if msg_id == 1:  # mcpInitializeID
            logger.bind(tag=TAG).debug("MCP initialize response received")
            server_info = result.get("serverInfo")
            if isinstance(server_info, dict):
                name = server_info.get("name")
                version = server_info.get("version")
                logger.bind(tag=TAG).debug(
                    f"Client MCP server: name={name}, version={version}"
                )

            await asyncio.sleep(1)
            logger.bind(tag=TAG).debug("Init done; requesting MCP tools/list")
            await send_mcp_tools_list_request(conn)

            return

        elif msg_id == 2:  # mcpToolsListID
            logger.bind(tag=TAG).debug("MCP tools/list response received")
            if isinstance(result, dict) and "tools" in result:
                ...
                logger.bind(tag=TAG).info(
                    f"Client tool count: {len(tools_data)}"
                )
                ...
                if next_cursor:
                    logger.bind(tag=TAG).debug(f"More tools, nextCursor: {next_cursor}")
                    await send_mcp_tools_list_continue_request(conn, next_cursor)
                else:
                    await mcp_client.set_ready(True)
                    logger.bind(tag=TAG).debug("All tools loaded; MCP client ready")
    ...
    # Handle method calls (requests from the client)
        ...
        logger.bind(tag=TAG).info(f"MCP client request: {method}")

    elif "error" in payload:
        error_data = payload["error"]
        error_msg = error_data.get("message", "unknown error")
        logger.bind(tag=TAG).error(f"MCP error response: {error_msg}")
...
async def send_mcp_initialize_message(conn: "ConnectionHandler"):
    ...
    logger.bind(tag=TAG).debug("Sending MCP initialize")
...
async def send_mcp_tools_list_request(conn: "ConnectionHandler"):
    ...
    logger.bind(tag=TAG).debug("Sending MCP tools/list")
    await send_mcp_message(conn, payload)
...
async def send_mcp_tools_list_continue_request(conn: "ConnectionHandler", cursor: str):
    ...
    logger.bind(tag=TAG).info(f"Sending MCP tools/list with cursor: {cursor}")
    await send_mcp_message(conn, payload)
...
async def call_mcp_tool(
                ...
                try:
                    ...
                    try:
                        ...
                        logger.bind(tag=TAG).error(
                            f"Args JSON parse failed: {str(e)}, raw: {args}"
                        )
    ...
    logger.bind(tag=TAG).info(f"MCP tools/call: {actual_name}, args: {args}")
    await send_mcp_message(conn, payload)
    ...
    try:
        ...
        logger.bind(tag=TAG).info(
            f"MCP tools/call {actual_name} ok, raw: {raw_result}"
        )
