    ...
    async def _worker(self, read_timeout_seconds: timedelta | None = None,
        ...
        async with AsyncExitStack() as stack:
            try:
                ...
                # 建立SSEClient
                    ...
                    if "API_ACCESS_TOKEN" in self.config:
                        ...
                        self.logger.bind(tag=TAG).warning(
                            "Deprecated API_ACCESS_TOKEN usage; set headers in .mcp_server_settings.json, e.g. 'Authorization': 'Bearer <token>'"
                        )
                ...
                self.logger.bind(tag=TAG).error(f"Server MCP client worker error: {e}")
                self._ready_evt.set()
                raise
