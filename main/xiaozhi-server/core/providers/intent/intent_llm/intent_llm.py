...
class IntentProvider(IntentProviderBase):
    ...
    async def detect_intent(
        ...
        logger.bind(tag=TAG).debug(f"Intent preprocess time: {preprocess_time:.4f}s")

        # 使用LLM进行意图识别
        llm_start_time = time.time()
        logger.bind(tag=TAG).debug(f"Starting LLM intent call, model: {model_info}")
        ...
        try:
            ...
            if "function_call" in intent_data:
                ...
                if function_name == "result_for_context":
                    ...
                    # 处理函数调用
                    logger.bind(tag=TAG).info(f"Intent function call: {function_name}")

            # 统一缓存处理和返回
            self.cache_manager.set(self.CacheType.INTENT, cache_key, intent)
            postprocess_time = time.time() - postprocess_start_time
            logger.bind(tag=TAG).debug(f"Intent postprocess time: {postprocess_time:.4f}s")
            return intent
