...
def _drop_ssl_env_if_file_missing() -> None:
    """
    Conda / tooling on Windows often sets SSL_CERT_FILE (or similar) to a path that
    does not exist; httpx then fails in ssl.create_default_context. Drop broken values
    so the default trust store is used.
    """
...
class ManageApiClient:
    ...
    def _init_client(cls, config):
        ...
        secret_raw = cls.config.get("secret")
        secret = "".join(str(secret_raw or "").split())
        if not cls.config.get("url") or not secret:
        ...
        if len(secret) > 4096:
            raise Exception(
                "manager-api.secret 过长，请只填写 server.secret（UUID），不要粘贴整段配置"
            )
        ...
        try:
            ...
            if result.get("code") == 10041:
                ...
                if result.get("code") == 401 or (
                    isinstance(msg, str) and "密钥" in msg
                ):
                    extra = (
                        " | Fix: copy current server.secret from 智控台 → 参数管理 "
                        "into xiaozhi-server/data/.config.yaml as manager-api.secret "
                        "(must match after DB reset / API reinstall)."
                    )
                raise Exception(f"API返回错误: {msg}{extra}")
...
async def generate_and_save_chat_summary(session_id: str) -> Optional[Dict]:
    ...
    try:
        ...
        print(f"Failed to generate/save chat summary: {e}")
        return None
