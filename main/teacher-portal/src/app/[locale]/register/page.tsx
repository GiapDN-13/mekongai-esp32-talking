...
import { backendFetch, encryptPassword } from '@/lib/api';
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!username || !password || !confirmPassword) { toast.error("Vui lòng điền đầy đủ thông tin"); return; }
    ...
    try {
      ...
      await backendFetch('/user/register', {
        method: 'POST',
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <Card className="w-full max-w-md shadow-lg">
          <CardHeader className="space-y-2 text-center">
            ...
            <CardDescription>Đăng ký để bắt đầu sử dụng robot trợ giảng</CardDescription>
          </CardHeader>
          <form onSubmit={handleRegister}>
            ...
            <CardFooter className="flex flex-col space-y-4">
              ...
              <div className="text-sm text-center text-muted-foreground">
                {t('hasAccount')}{" "}
                <Link href="/login" className="text-primary hover:underline font-medium">{t('loginButton')}</Link>
              </div>
            </CardFooter>
          </form>
        </Card>
