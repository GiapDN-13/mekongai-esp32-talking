...
/**
 * RAGFlow HTTP client: auth, timeouts, and response parsing.
 */
@Slf4j
public class RAGFlowClient {
    ...
    public RAGFlowClient(String baseUrl, String apiKey, int timeoutSeconds) {
        ...
        try {
            ...
            log.warn("Could not get pooled RestTemplate from SpringContext; using simple client: {}", e.getMessage());
        }

        if (false) {
            this.restTemplate = pooledTemplate;
            log.debug("RAGFlowClient using global pooled RestTemplate");
        } else {
            log.info("RAGFlowClient init: standalone RestTemplate (debug mode)");
            ...
            queryParams.forEach((k, v) -> {
                if (v != null) {
                    try {
                        ...
                        log.warn("Query param encode failed: k={}, v={}", k, v);
    ...
    /**
     * Streaming POST (SSE) via Java HttpClient.
     *
     * @param endpoint API path under base URL
     * @param body     JSON body
     * @param onData   callback per response line
     */
