...
@Tag(name = "Device management", description = "OTA endpoints")
@Slf4j
@RestController
...
public class OTAController {
    ...
    @Operation(summary = "OTA version and activation check")
    @PostMapping
    public ResponseEntity<String> checkOTAVersion(
            @RequestBody DeviceReportReqDTO deviceReportReqDTO,
            @Parameter(name = "Device-Id", description = "Device id (MAC)", required = true, in = ParameterIn.HEADER) @RequestHeader("Device-Id") String deviceId,
            @Parameter(name = "Client-Id", description = "Client id (defaults to Device-Id)", required = false, in = ParameterIn.HEADER) @RequestHeader(value = "Client-Id", required = false) String clientId) {
        ...
        // Device-Id must look like a valid MAC; full report still requires application in body elsewhere
        if (!macAddressValid) {
    ...
    @Operation(summary = "Quick activation probe")
    ...
    public ResponseEntity<String> getOTA() {
        ...
        if (StringUtils.isBlank(mqttUdpConfig)) {
            return ResponseEntity.ok("OTA unhealthy: server.mqtt_gateway is not set. Set it in admin console → system parameters.");
        }
        String wsUrl = sysParamsService.getValue(Constant.SERVER_WEBSOCKET, true);
        if (StringUtils.isBlank(wsUrl) || wsUrl.equals("null")) {
            return ResponseEntity.ok("OTA unhealthy: server.websocket is not set. Set it in admin console → system parameters.");
        }
        String otaUrl = sysParamsService.getValue(Constant.SERVER_OTA, true);
        if (StringUtils.isBlank(otaUrl) || otaUrl.equals("null")) {
            return ResponseEntity.ok("OTA unhealthy: server.ota is not set. Set it in admin console → system parameters.");
        }
        return ResponseEntity.ok("OTA OK; WebSocket endpoint count: " + wsUrl.split(";").length);
    }
    ...
    /**
     * Lenient MAC format check (colon or hyphen separated octets).
     */
    private boolean isMacAddressValid(String macAddress) {
        if (StringUtils.isBlank(macAddress)) {
            return false;
        }
        // Typical 6-octet MAC with : or - separators
