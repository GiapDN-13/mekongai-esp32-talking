...
public class ServerSideManageController {
    ...
    static {
        ...
        // Ignore JSON properties that have no matching fields on the POJO
        objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
    }

    @Operation(summary = "List WebSocket server URLs")
    ...
    @Operation(summary = "Notify Python server to reload configuration")
    @PostMapping("/emit-action")
    @LogOperation("Notify Python server to reload configuration")
    @RequiresPermissions("sys:role:superAdmin")
    public Result<Boolean> emitServerAction(@RequestBody @Valid EmitSeverActionDTO emitSeverActionDTO) {
        ...
        // Resolve target WebSocket from configured list
    ...
    private Boolean emitServerActionByWs(String targetWsUri, ServerActionEnum actionEnum) {
        ...
        String serverSK = sysParamsService.getValue(Constant.SERVER_SECRET, false);

        String deviceId = UUID.randomUUID().toString();
        String clientId = UUID.randomUUID().toString();

        String redisKey = xiaozhi.common.redis.RedisKeys.getTmpRegisterMacKey(deviceId);
        redisUtils.set(redisKey, "true", 300); // 5 minutes TTL
        ...
        try (WebSocketClientManager client = new WebSocketClientManager.Builder()
                ...
                .headers(headers)
                .build()) {
            // After connect, send JSON payload and wait for server response
            client.sendJson(
                    ServerActionPayloadDTO.build(
                            actionEnum,
                            Map.of("secret", serverSK)));
            // Block until predicate matches or session ends
            ...
            // Let global exception handler surface failures
            throw new RenException(ErrorCode.WEB_SOCKET_CONNECT_FAILED);
        }
        return true;
    }
