...
public class VoiceCloneServiceImpl extends BaseServiceImpl<VoiceCloneDao, VoiceCloneEntity>
    ...
    public void save(VoiceCloneDTO dto) {
        ...
        // For each selected voice ID, create one record
        int index = 0;
    ...
    public VoiceCloneResponseDTO getByIdWithNames(String id) {
        ...
        // Ensure trainStatus is set; the client uses it to detect cloned audio
    ...
    /**
     * Map a list of VoiceCloneEntity to VoiceCloneResponseDTO.
     */
    private List<VoiceCloneResponseDTO> convertToResponseDTOList(List<VoiceCloneEntity> entityList) {
        ...
        List<SysUserEntity> userList = userIdList.isEmpty() ? List.of() : sysUserRepository.findAllById(userIdList);
    ...
    /**
     * Call Volcano Engine (ByteDance) for voice cloning training.
     *
     * @param config model configuration
     * @param entity voice clone record
     * @throws Exception
     */
    private void huoshanClone(Map<String, Object> config, VoiceCloneEntity entity) throws Exception {
        ...
        if (baseResp != null) {
            ...
            if (statusCode != null && statusCode == 0 && StringUtils.isNotBlank(speakerId)) {
                ...
                // On failure, use StatusMessage as error text
