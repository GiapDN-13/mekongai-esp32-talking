...
public class AgentVoicePrintController {
    ...
    @PostMapping
    @Operation(summary = "Create voiceprint for an agent")
    ...
    @PutMapping
    @Operation(summary = "Update agent voiceprint")
    @RequiresPermissions("sys:role:normal")
    ...
    public Result<Void> delete(@PathVariable String id) {
        Long userId = SecurityUser.getUserId();
        // Service also clears related device bindings
    ...
    @GetMapping("/list/{id}")
    @Operation(summary = "List voiceprints for an agent (current user)")
    @RequiresPermissions("sys:role:normal")
